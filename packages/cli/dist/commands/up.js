...
import { addBaseComposeTunnelAgentService, commands, defaultVolumeSkipList, ensureMachine, findComposeTunnelAgentUrl, findEnvId, findProjectName, getTunnelNamesToServicePorts, jwkThumbprint, profileStore, telemetryEmitter, withSpinner, } from '@preevy/core';
...
const fetchTunnelServerDetails = async ({ log, tunnelingKey, envId, userModel, pStore, tunnelOpts, }) => {
    const expectedTunnels = getTunnelNamesToServicePorts(addBaseComposeTunnelAgentService(userModel), tunnelNameResolver({ envId }));
...
class Up extends MachineCreationDriverCommand {
    async run() {
        ...
        try {
            ...
            const tunnelingKey = await withSpinner(() => pStoreRef.tunnelingKey(), { text: 'Getting tunneling key from profile...', successText: 'Got tunneling key from profile' });
            ...
            await Promise.all(this.config.preevyHooks.envCreated.map(envCreated => envCreated({ log: this.logger, userModel }, { envId, urls })));
...
Up.description = 'Bring up a preview environment';
Up.flags = {
    ...envIdFlags,
    ...tunnelServerFlags,
    ...buildFlags,
