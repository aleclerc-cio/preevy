...
import { chooseFs, chooseFsType, fsTypes, isFsType } from '../../fs.js';
import { machineDrivers } from '../../drivers.js';
const validateFsType = (fsType) => {
    if (!isFsType(fsType)) {
        throw new Error(`Unsupported storage type: ${text.code(fsType)}. Supported types: ${text.codeList(fsTypes)}`);
    }
    return fsType;
};
const chooseTargetAlias = async (defaultAlias) => await inquirer.input({
    message: 'Target profile name',
    default: defaultAlias,
});
// eslint-disable-next-line no-use-before-define
class CopyProfile extends BaseCommand {
    async source(profileConfig) {
        const localProfileEntry = await profileConfig.get(this.flags.profile, { throwOnNotFound: false });
        if (!localProfileEntry) {
            throw new Error(`No current profile, specify the source alias with ${text.code(`--${_a.flags.profile.name}`)}`);
        }
...
_a = CopyProfile;
CopyProfile.description = 'Copy a profile';
CopyProfile.enableJsonFlag = true;
CopyProfile.flags = {
