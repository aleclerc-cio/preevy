...
import { isPartialMachine, machineResourceType } from '@preevy/core';
import DriverCommand from '../driver-command.js';
import { carefulBooleanPrompt } from '../prompt.js';
const isMachineResource = (r) => r.type === machineResourceType;
const confirmPurge = async ({ driverFriendlyName, envIds, groupedNonMachineResources, resourcePlurals }) => {
    const resources = [
...
class Purge extends DriverCommand {
    async run() {
        const { flags } = this;
        const driver = await this.driver();
        const resourcePlurals = { [machineResourceType]: 'machines', ...driver.resourcePlurals };
        const driverResourceTypes = new Set(Object.keys(resourcePlurals));
        flags.type.forEach(type => {
            if (!driverResourceTypes.has(type)) {
                ux.error(`Unknown resource type "${type}". Available resource types: ${Array.from(driverResourceTypes).join(', ')}`, { exit: 1 });
            }
        });
        const allResources = await asyncToArray(asyncFilter(({ type }) => flags.all || flags.type.includes(type), driver.listDeletableResources()));
        ...
        const groupedNonMachineResources = Object.entries(groupBy([...partialMachines, ...nonMachineResources], ({ type }) => type)).map(([type, resources]) => [type, resources.length]);
...
Purge.description = 'Delete all cloud provider machines and potentially other resources';
Purge.flags = {
    all: Flags.boolean({
