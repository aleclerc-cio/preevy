...
const pollTokensFromAuthEndpoint = async (loginUrl, deviceCode, logger, interval, clientId) => {
    try {
        while (true) {
            ...
            if (tokenResponse.status !== 403) {
                if (!tokenResponse.ok)
                    throw new Error(`Bad response from token endpoint: ${tokenResponse.status}: ${tokenResponse.statusText}`);
...
const deviceFlow = async (loginUrl, logger, clientId) => {
    ...
    return await withSpinner(() => pollTokensFromAuthEndpoint(loginUrl, responseData.device_code, logger, responseData.interval * 1000, clientId), {
        opPrefix: 'Waiting for approval',
        successText: 'Done!',
    });
};
export const getTokensFromLocalFs = async (fs) => {
    const tokensFile = await fs.read(PERSISTENT_TOKEN_FILE_NAME);
    if (tokensFile === undefined)
        return undefined;
    const tokens = JSON.parse(tokensFile.toString());
...
export const login = async (dataDir, loginUrl, lcUrl, clientId, logger) => {
    ...
    const postLoginResponse = await fetch(`${lcUrl}/api/cli/post-login`, {
        method: 'POST',
    ...
    if (postLoginResponse.ok) {
        ...
        if (!('currentOrg' in postLoginData)) {
            ...
            const createOrganizationResponse = await withSpinner(() => fetch(`${lcUrl}/api/org`, {
                method: 'POST',
                body: JSON.stringify({
                    name: orgName,
                    associateDomain,
