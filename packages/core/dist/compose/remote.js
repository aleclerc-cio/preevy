...
import { COMPOSE_TUNNEL_AGENT_SERVICE_NAME, formatPublicKey } from '@preevy/common';
import { composeModelFilename } from './model.js';
...
export const fetchRemoteUserModel = async (connection) => {
    const userModelStr = (await connection.exec(`cat ${REMOTE_DIR_BASE}/projects/*/${composeModelFilename}`)).stdout;
    return yaml.parse(userModelStr);
};
const serviceLinkEnvVars = (expectedServiceUrls) => Object.fromEntries(expectedServiceUrls
    .map(({ name, port, url }) => [`PREEVY_BASE_URI_${name.replace(/[^a-zA-Z0-9_]/g, '_')}_${port}`.toUpperCase(), url]));
export const defaultVolumeSkipList = [
    '/var/log',
    '/var/log/**',
    '/var/run',
    '/var/run/**',
...
const toPosix = (x) => x.split(path.sep).join(path.posix.sep);
const fixModelForRemote = async ({ skipServices = [], cwd, remoteBaseDir, volumeSkipList = defaultVolumeSkipList }, model) => {
    ...
    const overrideSecretsOrConfigs = (c) => mapValues(c ?? {}, secretOrConfig => {
...
export const remoteComposeModel = async ({ debug, userSpecifiedProjectName, userSpecifiedServices, volumeSkipList, composeFiles, log, cwd, expectedServiceUrls, projectName, agentSettings, modelFilter, }) => {
    ...
    const services = userSpecifiedServices.length
        ...
        : [];
    const { model: fixedModel, filesToCopy, skippedVolumes } = await fixModelForRemote({ cwd, remoteBaseDir: remoteDir, volumeSkipList }, await modelFilter(await composeClientWithInjectedArgs.getModel(services)));
    let model = fixedModel;
    if (agentSettings) {
        const { envId, machineStatusCommand, userAndGroup, scriptInjections, tunnelOpts, version, sshTunnelPrivateKey, allowedSshHostKeys, createCopiedFile, } = agentSettings;
