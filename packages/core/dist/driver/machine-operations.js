...
import { isPartialMachine, machineResourceType } from './machine-model.js';
const machineDiffText = (diff) => diff
    .map(({ name, old, new: n }) => `* ${name}: ${old} -> ${n}`).join(EOL);
const ensureBareMachine = async ({ machineDriver, machineCreationDriver, envId, log, debug, }) => {
...
const writeMetadata = async (machine, machineDriverName, driverOpts, connection, userAndGroup) => {
    const metadata = {
        machine: {
...
export const getUserAndGroup = async (connection) => (await connection.exec('echo "$(id -u):$(stat -c %g /var/run/docker.sock)"')).stdout
    .trim()
    .split(':');
export const getDockerPlatform = async (connection) => {
    const arch = (await connection.exec('docker info -f "{{.Architecture}}"')).stdout.trim();
    return arch === 'aarch64' ? 'linux/arm64' : 'linux/amd64';
};
const customizeNewMachine = ({ log, debug, envId, machine, machineDriver, machineCreationDriver, machineDriverName, initialConnection, }) => async (spinner) => {
    ...
    await retry(() => connection.exec('docker run hello-world'), {
        minTimeout: 2000,
        maxTimeout: 5000,
        retries: 5,
...
export const ensureMachine = async ({ machineDriver, machineCreationDriver, machineDriverName, envId, log, debug, }) => {
    const { machine, connection: connectionPromise, origin } = await ensureBareMachine({ machineDriver, machineCreationDriver, envId, log, debug });
    return await withSpinner(async (spinner) => {
...
//# sourceMappingURL=machine-operations.js.map