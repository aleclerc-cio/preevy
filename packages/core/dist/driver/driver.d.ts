...
export type MachineDriver<Machine extends MachineBase = MachineBase, ResourceType extends string = string> = {
    customizationScripts?: string[];
    friendlyName: string;
    resourcePlurals: Record<string, string>;
    getMachine: (args: {
        envId: string;
    }) => Promise<Machine | PartialMachine | undefined>;
    connect: (machine: MachineBase, opts: {
        log: Logger;
        debug: boolean;
    }) => Promise<MachineConnection>;
    spawnRemoteCommand: (machine: MachineBase, command: string[], stdio: PartialStdioOptions) => Promise<{
        code: number;
    } | {
        signal: string;
    }>;
...
export type MachineCreationDriver<Machine extends MachineBase = MachineBase> = {
    ...
    ensureMachineSnapshot: (args: {
        providerId: string;
        envId: string;
        wait: boolean;
    }) => Promise<void>;
    getMachineAndSpecDiff: (args: {
        envId: string;
    }) => Promise<(Machine & {
        specDiff: SpecDiffItem[];
    }) | PartialMachine | undefined>;
};
export type MachineDriverFactory<Flags, Machine extends MachineBase = MachineBase, ResourceType extends string = string> = ({ flags, profile, store, log, debug }: {
    flags: Flags;
    profile: Profile;
    store: Store;
    log: Logger;
